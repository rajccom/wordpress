...
//  php 8 deprication notice
            ...
            if (empty($file)) {
                ...
                if (substr($file, 0, 4) == 'php:' || substr($file, 0, 5) == 'data:' || substr($file, 0, 7) == 'base64:') {
                    ...
                    // this is the raw file data, base64-encoded, i.e.not uploaded
                        ...
                        $data = wpjobportalphplib::wpJP_safe_decoding(preg_replace('/^base64:(.*)/i', '$1', $file));
                        $file = 'base64';
    ...
    function getcolors($color) {
        $color = wpjobportalphplib::wpJP_str_replace('#', '', $color);
    ...
    function getoffsets($offsets, $x, $y, $round = true, $negative = true) {
        ...
        if (strpos($ct, '%')>0) $ct = $y * (wpjobportalphplib::wpJP_str_replace('%','',$ct) / 100);
        if (strpos($cr, '%')>0) $cr = $x * (wpjobportalphplib::wpJP_str_replace('%','',$cr) / 100);
        if (strpos($cb, '%')>0) $cb = $y * (wpjobportalphplib::wpJP_str_replace('%','',$cb) / 100);
        if (strpos($cl, '%')>0) $cl = $x * (wpjobportalphplib::wpJP_str_replace('%','',$cl) / 100);
        if (strpos($ct, 'px')>0) $ct = wpjobportalphplib::wpJP_str_replace('px','',$ct);
        if (strpos($cr, 'px')>0) $cr = wpjobportalphplib::wpJP_str_replace('px','',$cr);
        if (strpos($cb, 'px')>0) $cb = wpjobportalphplib::wpJP_str_replace('px','',$cb);
        if (strpos($cl, 'px')>0) $cl = wpjobportalphplib::wpJP_str_replace('px','',$cl);
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    if (!empty($this->image_text)) {
                        ...
                        $src_size_mb    = wpjobportalphplib::wpJP_number_format($src_size / 1024, 1, ".", " ");
                        $src_size_kb    = wpjobportalphplib::wpJP_number_format($src_size, 1, ".", " ");
                        $src_size_human = ($src_size > 1024 ? $src_size_mb . " MB" : $src_size_kb . " kb");

                        $this->image_text = wpjobportalphplib::wpJP_str_replace(
                            array('[src_name]',
                                  '[src_name_body]',
                                  '[src_name_ext]',
                                  '[src_pathname]',
                    ...
                    if ($this->image_reflection_height) {
                        ...
                        if (strpos($image_reflection_height, '%')>0) $image_reflection_height = $this->image_dst_y * (wpjobportalphplib::wpJP_str_replace('%','',$image_reflection_height / 100));
                        if (strpos($image_reflection_height, 'px')>0) $image_reflection_height = wpjobportalphplib::wpJP_str_replace('px','',$image_reflection_height);
