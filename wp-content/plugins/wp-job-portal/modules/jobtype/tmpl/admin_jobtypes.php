...
<div id="wpjobportaladmin-wrapper">
    ...
    <div id="wpjobportaladmin-data">
            ...
            <div id="wpjobportal-wrapper-top-left">
                <div id="wpjobportal-breadcrumbs">
                    <ul>
                        ...
                        <li><?php echo __('Job Types','wp-job-portal'); ?></li>
                    </ul>
                </div>
            </div>
        ...
        <div id="wpjobportal-admin-wrapper" class="p0 bg-n bs-n">
            ...
            <!-- filter form -->
            <form class="wpjobportal-filter-form" name="wpjobportalform" id="wpjobportalform" method="post" action="<?php echo admin_url("admin.php?page=wpjobportal_jobtype"); ?>">
            ...
            <?php
                if (!empty(wpjobportal::$_data[0])) {
                    ?>
                    <form id="wpjobportal-list-form" method="post" action="<?php echo admin_url("admin.php?page=wpjobportal_jobtype&task=saveordering"); ?>">
                                ...
                                <?php
                                    ...
                                    for ($i = 0, $n = count(wpjobportal::$_data[0]); $i < $n; $i++) {
                                        ...
                                        <tr id="id_<?php echo esc_attr($row->id);?>">
                                            ...
                                            <td>
                                                <a href="<?php echo admin_url('admin.php?page=wpjobportal_jobtype&wpjobportallt=formjobtype&wpjobportalid='.$row->id); ?>" title="<?php echo __('job types','wp-job-portal'); ?>">
                                                    <?php echo __(esc_html($row->title),'wp-job-portal'); ?>
                                                </a>
                                            </td>
                                            <td>
                                                <?php if ($row->isdefault == 1) { ?>
                                                    <img src="<?php echo WPJOBPORTAL_PLUGIN_URL; ?>includes/images/default.png" alt="<?php echo __('default','wp-job-portal'); ?>" border="0" />
                                                <?php } else { ?>
                                                    <a href="<?php echo esc_url(admin_url('admin.php?page=wpjobportal_common&task=makedefault&action=wpjobportaltask&for=jobtypes&id='.$row->id.$pageid)); ?>" title="<?php echo __('no default','wp-job-portal'); ?>">
                                                        <img src="<?php echo WPJOBPORTAL_PLUGIN_URL; ?>includes/images/notdefault.png" border="0" alt="<?php echo __('no default','wp-job-portal'); ?>" />
                                                    </a>
                                                <?php } ?>
                                            </td>
                                            <td>
                                                <?php if ($row->isactive == 1) { ?>
                                                    <a href="<?php echo esc_url(admin_url('admin.php?page=wpjobportal_jobtype&task=unpublish&action=wpjobportaltask&wpjobportal-cb[]='.$row->id.$pageid)); ?>" title="<?php echo __('published', 'wp-job-portal'); ?>">
                                                ...
                                                <?php } else { ?>
                                                    <a href="<?php echo esc_url(admin_url('admin.php?page=wpjobportal_jobtype&task=publish&action=wpjobportaltask&wpjobportal-cb[]='.$row->id.$pageid)); ?>" title="<?php echo __('not published', 'wp-job-portal'); ?>">
                                            ...
                                            <td>
                                                <a class="wpjobportal-table-act-btn" href="<?php echo esc_url(admin_url('admin.php?page=wpjobportal_jobtype&wpjobportallt=formjobtype&wpjobportalid='.$row->id)); ?>" title="<?php echo __('edit', 'wp-job-portal'); ?>">
                                                ...
                                                <a class="wpjobportal-table-act-btn" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=wpjobportal_jobtype&task=remove&action=wpjobportaltask&wpjobportal-cb[]='.$row->id),'delete-jobtype')); ?>" onclick='return confirmdelete("<?php echo __('Are you sure to delete', 'wp-job-portal').' ?'; ?>");' title="<?php echo __('delete', 'wp-job-portal'); ?>">
                        ...
                        <?php echo wp_kses(WPJOBPORTALformfield::hidden('_wpnonce', wp_create_nonce('delete-jobtype')),WPJOBPORTAL_ALLOWED_TAGS); ?>
            ...
            <?php
                if (wpjobportal::$_data[1]) {
                    ...
                    $link[] = array(
                           ...
                           'text' => __('Add New','wp-job-portal') .' '. __('Job Type','wp-job-portal')
                        );
